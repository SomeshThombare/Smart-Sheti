...
TREATMENT_ENGLISH = {
    ...
    "Potato__Late_Blight": "For late blight, spray metalaxyl plus mancozeb. Avoid excess moisture and improve field drainage.",
...
def preprocess_image(image_path):
    try:
        ...
        # Do not divide by 255 because EfficientNet preprocess_input
        # is already inside the trained model.
        ...
        raise ValueError("Invalid image. Please upload JPG, JPEG, or PNG image.")
...
def get_suggestion(class_name, confidence):
    if "healthy" in class_name.lower():
        return (
            ...
            "proper irrigation, and balanced fertilizer use."
        )

    if confidence < 50:
        return (
            "The prediction confidence is very low. Please upload a clear close-up "
            "leaf image with good lighting and consult an agriculture expert."
        )

    if confidence < 70:
        return (
            "The result has low confidence. Please upload another clear leaf image "
            "from a different angle or consult an agriculture expert."
        )

    return (
        "If the disease is severe or spreading quickly, consult a nearby agriculture "
        "officer or agriculture expert before applying chemicals."
    )
...
def predict_disease(image_path):
    ...
    if predicted_index >= len(class_names):
        raise ValueError("Prediction index is out of range. Check model and class_names.json.")
